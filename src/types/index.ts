...
export interface Contact {
  ...
  /** Stable WhatsApp user id (E.164 or LID like `CO.123…`). Migration 048.
   *  Used to dedupe / reply when Mercado Libre hides the phone. */
  wa_id?: string | null;
  /** Business-scoped user ID (`contacts[].user_id`). Migration 050. */
  bsuid?: string | null;
  /** WhatsApp username without leading @. Profile metadata only. */
...
/** Deal stub on a conversation row for lost-deal inbox filtering. */
export interface ConversationLinkedDeal {
  id: string;
  status: DealStatus;
...
export interface Conversation {
  ...
  /**
   * Contact deals (id/status/conversation_id) from the Inbox select.
   * Used to hide threads with a lost deal from agents while admins
   * keep full visibility.
   */
  linkedDeals?: ConversationLinkedDeal[];
  /**
